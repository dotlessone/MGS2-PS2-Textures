...
def main():
    ...
    print(f"[+] Found {len(csv_files)} CSV files. Merging based on first column...")

    merged = defaultdict(OrderedDict)
    all_headers = ["pcsx2_dumped_sha1"]  # renamed here

    # --- Load all CSVs and merge them ---
    for path in csv_files:
        header, rows = read_csv(path)
        if not header:
            print(f"  [!] Skipped empty file: {os.path.basename(path)}")
            continue
        ...
        for row in rows:
            ...
            for col, val in zip(header[1:], row[1:]):
                if val.strip() != "":
                    merged[key][col] = val.strip()

        print(f"  [+] Merged: {os.path.basename(path)} ({len(rows)} rows)")

    # --- Write merged output ---
    output_path = os.path.join(folder, "pcsx2_dumped_sha1_log.csv")
    with open(output_path, "w", newline="", encoding="utf-8") as out:
        ...
        for key in sorted(merged.keys(), key=str.lower):
            ...
            for col in all_headers[1:]:
                row.append(merged[key].get(col, "null"))
            writer.writerow(row)
